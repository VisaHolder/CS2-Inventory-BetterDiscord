...
var SETTINGS_SCHEMA = {
  ...
  watchThresholdPct: {
    ...
    description: "Watchlist alert sensitivity: get a Discord notice when a watched inventory's value moves at least this % from when you last saw it. Click the \u{1F514} on any card to watch/unwatch. Needs background refresh on.",
    default: 5
  },
  resetHistory: {
    type: OptionType.BOOLEAN,
...
function parseTradeHold(desc) {
  ...
  if (!tradable) for (const e of desc?.owner_descriptions ?? []) {
    ...
    if (m) {
      const t = Date.parse(m[1].replace(/\(.*?\)/g, "").trim());
      if (!isNaN(t)) {
        tradableAfter = t;
        break;
      }
...
async function loadInventory(steamId, opts) {
  ...
  const buildResult = () => {
    ...
    for (const [gk, g] of groups) {
      ...
      perItem.push({ name: g.phase ? `${g.name} (${g.phase})` : g.name, price: p, qty: g.qty, icon: g.icon, stickerValue: sm?.value, stickerCount: sm?.count, stickers: g.stickers.length ? g.stickers : void 0, rarity: g.rarity || void 0, hashName: g.name, float: ap?.float, seed: ap?.seed, nametag: ap?.nametag, catType: g.catType, inspect: ap?.inspect, assetid: g.qty === 1 ? g.assetids[0] : void 0, floatFlag, tradable: g.tradable, tradableAfter: g.tradableAfter });
    }
  ...
  if (wantCard) {
    ...
    card.addEventListener("click", (e) => {
      ...
      if (t?.closest?.(".vsi-watch")) {
        ...
        const bell = t.closest(".vsi-watch");
        bell.classList.toggle("on", now);
        bell.textContent = now ? "\u{1F514}" : "\u{1F515}";
        ...
        try {
          BD.UI?.showToast?.(now ? `Watching ${nm || "this inventory"} \u2014 alert on \xB1${settings.store.watchThresholdPct || 5}% moves` : "Stopped watching", { type: now ? "success" : "info" });
        } catch {
        }
        return;
      }
...
var WATCH_KEY = "vsi.watchlist";
var getWatchlist = () => {
  try {
    return BD.Data.load(PLUGIN_NAME, WATCH_KEY) ?? {};
  } catch {
    return {};
  }
};
var saveWatchlist = (w) => {
  try {
    BD.Data.save(PLUGIN_NAME, WATCH_KEY, w);
  } catch {
  }
};
var isWatched = (steamId) => !!getWatchlist()[steamId];
function toggleWatch(steamId, name, base, currency) {
...
function checkWatchAlerts() {
  ...
  for (const steamId of ids) {
    ...
    if (Math.abs(snap.total - e.base) / e.base >= thr) {
      ...
      try {
        BD.UI?.showNotice?.(`CS2 Inventory: ${e.name || "a watched inventory"} ${up ? "up" : "down"} ${pct}% \u2192 ${fmt(snap.total, e.currency)}`, { type: up ? "success" : "error" });
      } catch {
      }
      e.base = snap.total;
      changed = true;
...
async function backgroundTick() {
  ...
  try {
    ...
    const stale = trackedSteamIds().map((steamId) => ({ steamId, age: now - (getSnapshotsSync(steamId)[0]?.ts ?? 0) })).filter((x) => x.age > staleMs && (watched.has(x.steamId) || x.age < BG_RELEVANT_MS)).sort((a, b) => b.age - a.age).slice(0, BG_MAX_PER_TICK);
...
var RARITY_TIERS = {
  b0c3d9: { name: "Consumer", ord: 0 },
  "5e98d9": { name: "Industrial", ord: 1 },
  "4b69ff": { name: "Mil-Spec", ord: 2 },
  "8847ff": { name: "Restricted", ord: 3 },
  d32ce6: { name: "Classified", ord: 4 },
  eb4b4b: { name: "Covert", ord: 5 },
  e4ae39: { name: "Contraband", ord: 6 }
};
...
function renderPricedCard(card, latest, history, changed, steamId) {
...
async function openInventoryModal(steamId, displayName) {
  ...
  const render = () => {
    ...
    const filtered = items.filter((i) => (!query || abbrevItem(i.name).toLowerCase().includes(query)) && (!typeFilter || i.catType === typeFilter) && (!rarityFilter || i.rarity === rarityFilter));
    const byValue = (a, b) => b.price * b.qty - a.price * a.qty;
    filtered.sort((a, b) => {
    ...
    const rows = filtered.map((i, idx) => {
      ...
      const badge = i.stickerCount ? `<span class="vsi-modal-sticker${sv >= 50 ? " grail" : ""}" title="${escapeHtml(stickerNames || `${i.stickerCount} sticker${i.stickerCount > 1 ? "s" : ""}`)}">+${fmt(sv, cur)}</span>` : "";
      const hold = i.tradable === false ? `<span class="vsi-modal-hold" title="${i.tradableAfter ? `Trade hold \u2014 tradable ${new Date(i.tradableAfter).toLocaleDateString()}` : "On trade hold / not tradable"}">${i.tradableAfter && untilLabel(i.tradableAfter) ? `hold ${untilLabel(i.tradableAfter)}` : "locked"}</span>` : "";
      const pills = [
        hold,
        stTag(i.name),
